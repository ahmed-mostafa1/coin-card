...
class WalletService
{
    public function holdAmount(Wallet $wallet, string $amount, array $meta = [], bool $useTransaction = true): WalletTransaction
    {
        $operation = function () use ($wallet, $amount, $meta) {
            $lockedWallet = Wallet::whereKey($wallet->id)->lockForUpdate()->firstOrFail();

            DB::statement('update wallets set balance = balance - ?, held_balance = held_balance + ? where id = ?', [$amount, $amount, $lockedWallet->id]);

            $transaction = $lockedWallet->transactions()->create([
                'type' => $meta['type'] ?? WalletTransaction::TYPE_HOLD,
    ...
    public function settleHeldAmount(Wallet $wallet, string $amount, array $meta = [], bool $useTransaction = true): WalletTransaction
    {
        $operation = function () use ($wallet, $amount, $meta) {
            $lockedWallet = Wallet::whereKey($wallet->id)->lockForUpdate()->firstOrFail();

            DB::statement('update wallets set held_balance = held_balance - ? where id = ?', [$amount, $lockedWallet->id]);

            $transaction = $lockedWallet->transactions()->create([
                'type' => $meta['type'] ?? WalletTransaction::TYPE_SETTLE,
                'status' => $meta['status'] ?? WalletTransaction::STATUS_APPROVED,
                'amount' => $amount,
                'reference_type' => $meta['reference_type'] ?? null,
                'reference_id' => $meta['reference_id'] ?? null,
    ...
    public function releaseHeldAmount(Wallet $wallet, string $amount, array $meta = [], bool $useTransaction = true): WalletTransaction
    {
        $operation = function () use ($wallet, $amount, $meta) {
            ...
            DB::statement('update wallets set held_balance = held_balance - ?, balance = balance + ? where id = ?', [$amount, $amount, $lockedWallet->id]);

            $transaction = $lockedWallet->transactions()->create([
                'type' => $meta['type'] ?? WalletTransaction::TYPE_RELEASE,
                'status' => $meta['status'] ?? WalletTransaction::STATUS_APPROVED,
                'amount' => $amount,
                'reference_type' => $meta['reference_type'] ?? null,
                'reference_id' => $meta['reference_id'] ?? null,
