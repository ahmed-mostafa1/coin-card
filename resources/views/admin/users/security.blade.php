    ...
    <div class="space-y-6">
        <x-card :hover="false" class="p-8">
            <x-page-header title="سجلات الأمان: {{ $user->name }}" subtitle="كل النشاط التقني والأمني المرتبط بالمستخدم.">
                <a href="{{ route('admin.users.show', $user) }}" class="rounded-xl border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-600">عودة للمستخدم</a>
            </x-page-header>

            <form method="GET" class="mt-6 grid gap-3 md:grid-cols-4">
                <input type="date" name="from" value="{{ request('from') }}" class="rounded-xl border border-slate-200 px-3 py-2 text-sm">
                <input type="date" name="to" value="{{ request('to') }}" class="rounded-xl border border-slate-200 px-3 py-2 text-sm">
                <select name="action" class="rounded-xl border border-slate-200 px-3 py-2 text-sm">
                    <option value="">كل الإجراءات</option>
                    @foreach ($actions as $action)
                        <option value="{{ $action }}" @selected(request('action') === $action)>{{ $action }}</option>
                    @endforeach
                </select>
                <input type="text" name="ip" value="{{ request('ip') }}" placeholder="IP" class="rounded-xl border border-slate-200 px-3 py-2 text-sm">
                <input type="text" name="country" value="{{ request('country') }}" placeholder="Country code" class="rounded-xl border border-slate-200 px-3 py-2 text-sm">
                <select name="device" class="rounded-xl border border-slate-200 px-3 py-2 text-sm">
                ...
                <input type="number" name="order_id" value="{{ request('order_id') }}" placeholder="رقم الطلب" class="rounded-xl border border-slate-200 px-3 py-2 text-sm">
                <input type="number" name="deposit_request_id" value="{{ request('deposit_request_id') }}" placeholder="رقم الشحن" class="rounded-xl border border-slate-200 px-3 py-2 text-sm">
        ...
        <x-card :hover="false" class="p-6">
            ...
            <x-table class="mt-4">
                ...
                <tbody>
                    ...
                    @empty
                        <tr><td colspan="7" class="py-6 text-center text-slate-500">لا توجد سجلات.</td></tr>
                    @endforelse
                </tbody>
            </x-table>
        ...
        <div class="grid gap-6 lg:grid-cols-3">
            <x-card :hover="false" class="p-6">
                ...
                <div class="mt-4 space-y-2 text-sm">
                    ...
                    @empty
                        <p class="text-slate-500">لا توجد بيانات.</p>
                    @endforelse
                </div>
            </x-card>
            <x-card :hover="false" class="p-6">
                ...
                <div class="mt-4 space-y-2 text-sm">
                    ...
                    @empty
                        <p class="text-slate-500">لا توجد محاولات.</p>
                    @endforelse
                </div>
            </x-card>
            <x-card :hover="false" class="p-6">
                ...
                <div class="mt-4 space-y-2 text-sm">
                    ...
                    @empty
                        <p class="text-slate-500">لا توجد تنبيهات.</p>
                    @endforelse
                </div>
            </x-card>
        </div>
