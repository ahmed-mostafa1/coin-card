...
@section('content')
    <div class="rounded-3xl border border-emerald-100 dark:border-emerald-800 bg-white dark:bg-slate-800 p-8 shadow-sm">
        <div class="mb-6 flex items-center justify-between">
            <h1 class="text-2xl font-semibold text-emerald-700 dark:text-emerald-400">{{ __('messages.edit_field') }}: {{ $field->label }}</h1>
        ...
        <form method="POST" action="{{ route('admin.agency-request-fields.update', $field) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                <div>
                    ...
                    <x-select id="type" name="type" required>
                        ...
                        <option value="number" @selected(old('type', $field->type) === 'number')>{{ __('messages.field_type_number') }}</option>
                        <option value="email" @selected(old('type', $field->type) === 'email')>{{ __('messages.field_type_email') }}</option>
                        <option value="tel" @selected(old('type', $field->type) === 'tel')>{{ __('messages.field_type_tel') }}</option>
                    </x-select>
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                <div class="flex items-center gap-3 text-sm text-slate-600 dark:text-slate-400">
                    <input id="is_required" name="is_required" type="checkbox" value="1" class="rounded border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-emerald-600 focus:ring-emerald-500 dark:focus:ring-emerald-600" {{ old('is_required', $field->is_required) ? 'checked' : '' }}>
