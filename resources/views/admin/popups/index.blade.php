...
@section('content')
    <div class="rounded-3xl border border-emerald-100 dark:border-emerald-800 bg-white dark:bg-slate-800 p-8 shadow-sm">
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-semibold text-emerald-900 dark:text-emerald-100">النوافذ المنبثقة</h1>
        ...
        <x-table class="mt-6">
            ...
            <tbody class="divide-y divide-slate-50 dark:divide-slate-700">
                @forelse ($popups as $popup)
                    <tr>
                        ...
                        <td class="py-3">
                            ...
                            @else
                                <span class="text-sm text-slate-100">لا توجد صورة</span>
                            @endif
                        </td>
                        <td class="py-3">
                            @if ($popup->is_active)
                        ...
                        <td class="py-3 text-slate-900 dark:text-slate-100">{{ $popup->display_order }}</td>
                        <td class="py-3">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('admin.popups.edit', $popup) }}" class="text-emerald-900 dark:text-emerald-100 hover:text-emerald-900 dark:hover:text-emerald-100">تعديل</a>
                ...
                @empty
                    <tr>
                        <td colspan="5" class="py-6 text-center text-slate-900 dark:text-slate-100">لا توجد نوافذ منبثقة بعد.</td>
                    </tr>
                @endforelse
            </tbody>
        </x-table>
