    ...
    <div class="w-full rounded-3xl border border-emerald-100 dark:border-slate-700 bg-white dark:bg-slate-800 p-8 shadow-sm transition-colors duration-200">
        ...
        <form method="POST" action="{{ route('admin.popups.store') }}" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                ...
                <textarea id="content" name="content" rows="4" class="w-full rounded-xl border border-slate-50 dark:border-slate-900 bg-white dark:bg-slate-700 px-4 py-3 text-sm text-slate-700 dark:text-slate-50 focus:border-emerald-500 focus:ring-emerald-500">{{ old('content') }}</textarea>
                <x-input-error :messages="$errors->get('content')" />
            </div>

            <div>
                <x-input-label for="content_en" value="المحتوى (إنجليزي)" />
                <textarea id="content_en" name="content_en" rows="4" class="w-full rounded-xl border border-slate-50 dark:border-slate-900 bg-white dark:bg-slate-700 px-4 py-3 text-sm text-slate-700 dark:text-slate-50 focus:border-emerald-500 focus:ring-emerald-500">{{ old('content_en') }}</textarea>
            ...
            <div class="rounded-xl border border-slate-50 dark:border-slate-900 p-4 space-y-4">
                ...
                <div class="flex flex-wrap gap-4">
                    <div class="flex items-center gap-2">
                        ...
                        <input id="button_color" name="button_color" type="color" value="{{ old('button_color', '#10b981') }}" class="h-9 w-14 rounded cursor-pointer border border-slate-50 dark:border-slate-900">
                        <x-input-error :messages="$errors->get('button_color')" />
                    </div>
                    <div class="flex items-center gap-2">
                        <x-input-label for="button_text_color" value="لون النص" />
                        <input id="button_text_color" name="button_text_color" type="color" value="{{ old('button_text_color', '#ffffff') }}" class="h-9 w-14 rounded cursor-pointer border border-slate-50 dark:border-slate-900">
