...
@section('content')
    <div class="w-full rounded-3xl border border-emerald-100 dark:border-slate-700 bg-white dark:bg-slate-800 p-8 shadow-sm transition-colors duration-200">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-semibold text-emerald-900 dark:text-emerald-400">تعديل نافذة منبثقة</h1>
        ...
        <form method="POST" action="{{ route('admin.popups.update', $popup) }}" enctype="multipart/form-data" class="space-y-4">
            ...
            <div>
                ...
                <textarea id="content" name="content" rows="4" class="w-full rounded-xl border border-slate-50 dark:border-slate-900 bg-white dark:bg-slate-700 px-4 py-3 text-sm text-slate-700 dark:text-slate-50 focus:border-emerald-500 focus:ring-emerald-500">{{ old('content', $popup->content) }}</textarea>
                <x-input-error :messages="$errors->get('content')" />
            </div>

            <div>
                <x-input-label for="content_en" value="المحتوى (إنجليزي)" />
                <textarea id="content_en" name="content_en" rows="4" class="w-full rounded-xl border border-slate-50 dark:border-slate-900 bg-white dark:bg-slate-700 px-4 py-3 text-sm text-slate-700 dark:text-slate-50 focus:border-emerald-500 focus:ring-emerald-500">{{ old('content_en', $popup->content_en) }}</textarea>
            ...
            <div>
                ...
                @if($popup->image_path)
                    <div class="mb-2">
                        <img src="{{ asset('storage/' . $popup->image_path) }}" alt="Current Image" class="h-20 w-auto rounded-lg object-contain bg-slate-50 dark:bg-slate-700 p-1">
                    </div>
                @endif
            ...
            <div class="rounded-xl border border-slate-50 dark:border-slate-900 p-4 space-y-4">
                <p class="text-sm font-semibold text-slate-900 dark:text-slate-50">زر الإجراء (اختياري)</p>
                ...
                <div class="flex flex-wrap gap-4">
                    <div class="flex items-center gap-2">
                        ...
                        <input id="button_color" name="button_color" type="color" value="{{ old('button_color', $popup->button_color ?? '#10b981') }}" class="h-9 w-14 rounded cursor-pointer border border-slate-50 dark:border-slate-900">
                        <x-input-error :messages="$errors->get('button_color')" />
                    </div>
                    <div class="flex items-center gap-2">
                        <x-input-label for="button_text_color" value="لون النص" />
                        <input id="button_text_color" name="button_text_color" type="color" value="{{ old('button_text_color', $popup->button_text_color ?? '#ffffff') }}" class="h-9 w-14 rounded cursor-pointer border border-slate-50 dark:border-slate-900">
            ...
            <div class="flex items-center gap-3 text-sm text-slate-900 dark:text-slate-400">
                <input id="is_active" name="is_active" type="checkbox" value="1" class="rounded border-slate-50 dark:border-slate-900 bg-white dark:bg-slate-700 text-emerald-600 focus:ring-emerald-500" {{ $popup->is_active ? 'checked' : '' }}>
            ...
            <div class="flex gap-3 pt-4">
                ...
                <a href="{{ route('admin.popups.index') }}" class="rounded-full border border-slate-50 dark:border-slate-900 px-4 py-2 text-sm font-semibold text-slate-900 dark:text-slate-50 transition hover:border-emerald-200 dark:hover:border-emerald-500">إلغاء</a>
            </div>
        </form>
    </div>
@endsection
