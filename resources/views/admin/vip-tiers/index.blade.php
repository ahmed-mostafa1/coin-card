...
@section('content')
    <div class="rounded-3xl border border-emerald-100 dark:border-emerald-800 bg-white dark:bg-slate-800 p-8 shadow-sm">
        <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-semibold text-emerald-900 dark:text-emerald-100">مستويات VIP</h1>
        ...
        <x-table class="mt-6">
            ...
            <tbody class="divide-y divide-slate-50 dark:divide-slate-700">
                @forelse ($tiers as $tier)
                    <tr>
                        <td class="py-3 text-slate-900 dark:text-slate-100">{{ $tier->rank }}</td>
                        <td class="py-3">
                            @if($tier->image_path)
                                <img src="{{ asset('storage/' . $tier->image_path) }}" alt="{{ $tier->title_ar }}" class="h-10 w-10 rounded-full object-cover">
                            @else
                                <span class="text-sm text-slate-100">لا يوجد</span>
                            @endif
                        </td>
                        ...
                        <td class="py-3 text-emerald-600 dark:text-emerald-100 font-bold">$ {{ number_format($tier->deposits_required, 2) }}</td>
                        <td class="py-3">
                            @if ($tier->is_active)
                        ...
                        <td class="py-3">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('admin.vip-tiers.edit', $tier) }}" class="text-emerald-900 dark:text-emerald-100 hover:text-emerald-900 dark:hover:text-emerald-100">تعديل</a>
                ...
                @empty
                    <tr>
                        <td colspan="7" class="py-6 text-center text-slate-900 dark:text-slate-100">لا توجد مستويات بعد.</td>
                    </tr>
                @endforelse
            </tbody>
        </x-table>
