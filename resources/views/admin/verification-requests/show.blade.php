...
@section('content')
    <div class="grid gap-6 lg:grid-cols-3">
        <x-card :hover="false" class="p-8 lg:col-span-2">
            ...
            @if (session('status'))<div class="mt-6 rounded-xl border border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-900/30 px-4 py-3 text-sm text-emerald-700 dark:text-emerald-300">{{ session('status') }}</div>@endif
            <div class="mt-6 space-y-3">
                @foreach (($verificationRequest->payload ?? []) as $key => $value)
                    <div class="rounded-2xl border border-slate-200 p-4 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/70"><p class="text-xs text-slate-500 dark:text-slate-400">{{ $key }}</p><p class="mt-1 font-semibold text-slate-800 dark:text-white break-words">{{ $value }}</p></div>
                @endforeach
                @foreach ($verificationRequest->files as $file)
                    <div class="rounded-2xl border border-slate-200 p-4 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/70"><p class="text-xs text-slate-500 dark:text-slate-400">{{ $file->field?->label ?? $file->field_key }}</p><a href="{{ route('admin.verification-requests.files.show', [$verificationRequest, $file->id]) }}" class="mt-2 inline-flex rounded-full border border-emerald-200 dark:border-emerald-700 px-4 py-2 text-sm font-semibold text-emerald-700 dark:text-emerald-300">عرض/تحميل الملف</a></div>
                @endforeach
            </div>
        </x-card>
        <x-card :hover="false" class="p-8">
            ...
            <form method="POST" action="{{ route('admin.verification-requests.approve', $verificationRequest) }}" class="mt-4 space-y-4">
                ...
                <textarea name="review_note" rows="3" class="w-full rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 px-4 py-2 text-sm text-slate-700 dark:text-slate-200" placeholder="ملاحظة المراجعة">{{ old('review_note') }}</textarea>
            ...
            <form method="POST" action="{{ route('admin.verification-requests.status', $verificationRequest) }}" class="mt-6 space-y-4">
                ...
                <textarea name="review_note" rows="3" class="w-full rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 px-4 py-2 text-sm text-slate-700 dark:text-slate-200" placeholder="ملاحظة للمستخدم"></textarea>
