    ...
    <div class="grid gap-6 lg:grid-cols-3">
        ...
        <x-card :hover="false" class="p-8">
            ...
            <form method="POST" action="{{ route('admin.verification-requests.approve', $verificationRequest) }}" class="mt-4 space-y-4">
                ...
                <textarea name="review_note" rows="3" class="w-full rounded-xl border border-slate-50 dark:border-slate-900 bg-white dark:bg-slate-700 px-4 py-2 text-sm text-slate-700 dark:text-slate-50" placeholder="ملاحظة المراجعة">{{ old('review_note') }}</textarea>
            ...
            <form method="POST" action="{{ route('admin.verification-requests.status', $verificationRequest) }}" class="mt-6 space-y-4">
                ...
                <textarea name="review_note" rows="3" class="w-full rounded-xl border border-slate-50 dark:border-slate-900 bg-white dark:bg-slate-700 px-4 py-2 text-sm text-slate-700 dark:text-slate-50" placeholder="ملاحظة للمستخدم"></textarea>
