...
@section('content')
    <div class="grid gap-6 lg:grid-cols-3">
        ...
        <div class="rounded-3xl border border-emerald-100 dark:border-slate-700 bg-white dark:bg-slate-800 p-8 shadow-sm">
            ...
            @if ($depositRequest->status === 'pending')
                <form method="POST" action="{{ route('admin.deposits.approve', $depositRequest) }}" class="mt-6 space-y-4">
                    ...
                    <div>
                        ...
                        <textarea id="admin_note" name="admin_note" rows="3" class="w-full rounded-xl border border-slate-50 dark:border-slate-900 bg-white/80 dark:bg-slate-700 px-4 py-2 text-sm text-slate-700 dark:text-slate-50">{{ old('admin_note') }}</textarea>
                ...
                <form method="POST" action="{{ route('admin.deposits.reject', $depositRequest) }}" class="mt-6 space-y-4">
                    ...
                    <div>
                        ...
                        <textarea id="reject_note" name="admin_note" rows="3" class="w-full rounded-xl border border-slate-50 dark:border-slate-900 bg-white/80 dark:bg-slate-700 px-4 py-2 text-sm text-slate-700 dark:text-slate-50">{{ old('admin_note') }}</textarea>
                        <x-input-error :messages="$errors->get('admin_note')" />
                    </div>
            ...
            @else
                <div class="mt-6 rounded-2xl border border-slate-50 dark:border-slate-700 bg-white dark:bg-slate-900 p-4 text-sm text-slate-900 dark:text-slate-50">
