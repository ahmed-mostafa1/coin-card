...
@section('content')
    <div class="space-y-6">
        <div class="rounded-3xl border border-emerald-100 bg-white p-8 shadow-sm">
            <h1 class="text-2xl font-semibold text-emerald-700">إرسال إشعار لجميع المستخدمين</h1>
            <p class="mt-2 text-sm text-slate-600">سيتم إرسال هذا الإشعار عبر البريد الإلكتروني ويظهر في الموقع لجميع المستخدمين.</p>
            ...
            <form method="POST" action="{{ route('admin.notifications.store') }}" class="mt-6 space-y-6">
                ...
                <div class="grid gap-6 md:grid-cols-2">
                    <div>
                        ...
                        <textarea id="content_ar" name="content_ar" rows="4" class="mt-1 w-full rounded-xl border border-slate-200 bg-white/80 px-4 py-2 text-sm text-slate-700 focus:border-emerald-500 focus:ring-emerald-500" required>{{ old('content_ar') }}</textarea>
                        <x-input-error :messages="$errors->get('content_ar')" />
                    </div>
                    <div>
                        <x-input-label for="content_en" value="المحتوى (إنجليزي)" />
                        <textarea id="content_en" name="content_en" rows="4" class="mt-1 w-full rounded-xl border border-slate-200 bg-white/80 px-4 py-2 text-sm text-slate-700 focus:border-emerald-500 focus:ring-emerald-500" required>{{ old('content_en') }}</textarea>
