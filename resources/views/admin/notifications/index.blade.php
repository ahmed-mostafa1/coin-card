            ...
            @forelse ($sentNotifications as $notification)
                <div class="rounded-2xl border border-slate-200 p-4 dark:border-slate-700">
                    ...
                    @if ($notification->image_path)
                        <img src="{{ asset('storage/'.$notification->image_path) }}" alt="notification" class="mt-3 h-32 w-auto rounded-lg border border-slate-200 object-contain">
                    @endif
                </div>
            @empty
                <p class="text-sm text-slate-500">لا توجد إشعارات مرسلة.</p>
            @endforelse
        </div>
