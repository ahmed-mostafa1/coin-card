...
<div><x-input-label for="options_text" value="خيارات القائمة (كل خيار في سطر)" /><textarea id="options_text" name="options_text" rows="4" class="w-full rounded-xl border border-slate-50 dark:border-slate-900 bg-white dark:bg-slate-700 px-4 py-2 text-sm text-slate-700 dark:text-white">{{ old('options_text', implode("\n", $field?->options ?? [])) }}</textarea></div>
<div class="grid gap-4 md:grid-cols-2">
...
<div class="grid gap-4 md:grid-cols-3">
    ...
    <label class="flex items-center gap-2 text-sm text-slate-900"><input type="checkbox" name="is_required" value="1" class="rounded border-slate-50 text-emerald-600" @checked(old('is_required', $field?->is_required ?? true))> مطلوب</label>
    <label class="flex items-center gap-2 text-sm text-slate-900"><input type="checkbox" name="is_enabled" value="1" class="rounded border-slate-50 text-emerald-600" @checked(old('is_enabled', $field?->is_enabled ?? true))> مفعل</label>
</div>
<div class="flex gap-3"><x-primary-button>{{ __('messages.save') }}</x-primary-button><a href="{{ route('admin.verification-fields.index') }}" class="rounded-full border border-slate-50 px-4 py-2 text-sm font-semibold text-slate-900 dark:text-slate-50">{{ __('messages.cancel') }}</a></div>
