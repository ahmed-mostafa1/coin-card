    ...
    <div class="space-y-6">
        <div class="rounded-3xl border border-emerald-100 dark:border-emerald-800 bg-white dark:bg-slate-800 p-8 shadow-sm">
            ...
            <div class="mt-4 flex flex-wrap gap-2 text-xs">
                <a href="{{ route('admin.reports.index', ['preset' => 'today']) }}"
                    class="rounded-full border px-3 py-1 transition {{ $preset === 'today' ? 'border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-900 dark:text-emerald-400' : 'border-slate-50 dark:border-slate-900 text-slate-900 dark:text-slate-50 hover:border-emerald-200 dark:hover:border-emerald-700' }}">اليوم</a>
                <a href="{{ route('admin.reports.index', ['preset' => '7']) }}"
                    class="rounded-full border px-3 py-1 transition {{ $preset === '7' ? 'border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-900 dark:text-emerald-400' : 'border-slate-50 dark:border-slate-900 text-slate-900 dark:text-slate-50 hover:border-emerald-200 dark:hover:border-emerald-700' }}">آخر 7 أيام</a>
                <a href="{{ route('admin.reports.index', ['preset' => '30']) }}"
                    class="rounded-full border px-3 py-1 transition {{ $preset === '30' ? 'border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-900 dark:text-emerald-400' : 'border-slate-50 dark:border-slate-900 text-slate-900 dark:text-slate-50 hover:border-emerald-200 dark:hover:border-emerald-700' }}">آخر 30 يوم</a>
            </div>
        </div>
        ...
        <div class="grid gap-6 lg:grid-cols-3">
            ...
            <div class="rounded-3xl border border-emerald-100 dark:border-emerald-800 bg-white dark:bg-slate-800 p-6 shadow-sm">
                ...
                <div class="mt-4 space-y-3 text-sm">
                    <div class="flex items-center justify-between">
                        <span class="text-slate-900 dark:text-slate-400">الرصيد المتاح</span>
                        <span class="font-semibold text-slate-700 dark:text-white">{{ number_format($totalBalance, 2) }} USD</span>
                    </div>
                    <div class="flex items-center justify-between">
                        <span class="text-slate-900 dark:text-slate-400">الرصيد المعلّق</span>
        ...
        <div class="grid gap-6 lg:grid-cols-2">
            <div class="rounded-3xl border border-emerald-100 dark:border-emerald-800 bg-white dark:bg-slate-800 p-6 shadow-sm">
                ...
                <div class="mt-4 space-y-3 text-sm">
                    ...
                    @empty
                        <p class="text-sm text-slate-900 dark:text-slate-400">لا توجد بيانات كافية للفترة.</p>
                    @endforelse
                </div>
            </div>
            ...
            <div class="rounded-3xl border border-emerald-100 dark:border-emerald-800 bg-white dark:bg-slate-800 p-6 shadow-sm">
                ...
                <div class="mt-4 space-y-3 text-sm">
                    @forelse ($topUsers as $user)
                        <div class="flex items-center justify-between">
                            <span class="text-slate-900 dark:text-slate-50">{{ $user->name }}<span class="text-xs text-slate-400 dark:text-slate-900"> ({{ $user->email }})</span></span>
