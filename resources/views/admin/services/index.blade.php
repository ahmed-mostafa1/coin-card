    ...
    <div class="rounded-3xl border border-emerald-100 dark:border-slate-700 bg-white dark:bg-slate-800 p-8 shadow-sm transition-colors duration-200">
        <div class="flex flex-wrap items-center justify-between gap-4">
            ...
            <div class="flex flex-wrap items-center gap-4">
                <form action="{{ route('admin.services.index') }}" method="GET" class="relative">
                    <div class="relative">
                        <input type="text" 
                               ...
                               class="w-64 rounded-xl border border-slate-200 bg-slate-50 py-2.5 pr-10 pl-4 text-sm outline-none transition focus:border-emerald-500 focus:bg-white focus:ring-1 focus:ring-emerald-500 dark:border-slate-700 dark:bg-slate-900/50 dark:text-white dark:focus:border-emerald-500 dark:focus:bg-slate-900">
                ...
                @if(request('search'))
                    <a href="{{ route('admin.services.index') }}" class="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-50 hover:text-rose-600 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300 dark:hover:text-rose-400 transition-colors">
                        إلغاء البحث
                    </a>
                @endif
                <a href="{{ route('admin.services.create') }}" class="rounded-full bg-emerald-600 px-6 py-2.5 text-sm font-semibold text-white shadow-sm shadow-emerald-200 transition hover:bg-emerald-700 hover:shadow-emerald-300 dark:shadow-none">{{ __('messages.add_service') }}</a>
            </div>
        </div>
