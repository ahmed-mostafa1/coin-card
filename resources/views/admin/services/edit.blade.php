...
@section('content')
    <div class="grid gap-6 lg:grid-cols-3">
        <div class="rounded-3xl border border-emerald-100 dark:border-slate-700 bg-white dark:bg-slate-800 p-8 shadow-sm lg:col-span-2 transition-colors duration-200">
            ...
            @if (session('status'))
                <div class="mt-6 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700 dark:border-emerald-600 dark:bg-emerald-900/20 dark:text-emerald-300">
                    {{ session('status') }}
                </div>
            @endif
            ...
            <form method="POST" action="{{ route('admin.services.update', $service) }}" enctype="multipart/form-data" class="mt-6 space-y-4">
                ...
                <div class="flex items-start gap-3 text-sm text-slate-600 dark:text-slate-400">
                    <input id="is_featured" name="is_featured" type="checkbox" value="1" class="mt-1 rounded border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 text-emerald-600 focus:ring-emerald-500" {{ old('is_featured', $service->is_featured) ? 'checked' : '' }}>
                    <div class="space-y-1">
        ...
        <div class="space-y-6">
            ...
            <div class="rounded-3xl border border-emerald-100 dark:border-slate-700 bg-white dark:bg-slate-800 p-6 shadow-sm transition-colors duration-200">
                <div class="flex items-center justify-between">
                    <h2 class="text-lg font-semibold text-emerald-700 dark:text-emerald-400">{{ __('messages.service_buttons') }}</h2>
                    <a href="{{ route('admin.services.buttons.create', $service) }}" class="text-sm text-emerald-700 dark:text-emerald-400 hover:text-emerald-800 dark:hover:text-emerald-300">+ {{ __('messages.add_button') }}</a>
                </div>
                <div class="mt-4 space-y-3">
