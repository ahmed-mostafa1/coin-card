...
@section('content')
    <div class="w-full rounded-3xl border border-emerald-100 dark:border-slate-700 bg-white dark:bg-slate-800 p-8 shadow-sm transition-colors duration-200">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-semibold text-emerald-900 dark:text-emerald-100">{{ __('messages.edit_button') }}</h1>
            <a href="{{ route('admin.services.edit', $service) }}" class="text-sm text-emerald-900 dark:text-emerald-100 hover:text-emerald-800 dark:hover:text-emerald-100">{{ __('messages.back_to_service') }}</a>
        </div>
        ...
        <form method="POST" action="{{ route('admin.services.buttons.update', [$service, $button]) }}" class="mt-6 space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <div class="flex items-center gap-3">
                    <input id="bg_color" name="bg_color" type="color" value="{{ old('bg_color', $button->bg_color) }}"
                        class="h-10 w-16 cursor-pointer rounded-lg border border-slate-50 dark:border-slate-900 bg-white dark:bg-slate-700 p-1">
            ...
            <div class="flex gap-3 pt-2">
                ...
                <a href="{{ route('admin.services.edit', $service) }}"
                    class="rounded-full border border-slate-50 dark:border-slate-900 px-4 py-2 text-sm font-semibold text-slate-900 dark:text-slate-50 hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors">
                    {{ __('messages.cancel') }}
                </a>
            </div>
        </form>

        <div class="mt-6 border-t border-slate-50 dark:border-slate-700 pt-4">
            <form method="POST" action="{{ route('admin.services.buttons.destroy', [$service, $button]) }}"
