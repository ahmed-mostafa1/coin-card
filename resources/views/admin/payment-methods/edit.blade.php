...
@section('content')
    <div class="w-full rounded-3xl border border-emerald-100 bg-white p-8 shadow-sm">
        <h1 class="text-2xl font-semibold text-emerald-700">{{ __('messages.edit_payment_method') }}</h1>
        <p class="mt-2 text-sm text-slate-600">{{ __('messages.update_payment_method_details') }}</p>
        ...
        <form method="POST" action="{{ route('admin.payment-methods.update', $paymentMethod) }}" enctype="multipart/form-data" class="mt-6 space-y-4">
            ...
            <div>
                ...
                <textarea id="instructions" name="instructions" rows="5" class="w-full rounded-xl border border-slate-200 bg-white/80 px-4 py-2 text-sm text-slate-700">{{ old('instructions', $paymentMethod->instructions) }}</textarea>
                <x-input-error :messages="$errors->get('instructions')" />
            </div>

            <div>
                <x-input-label for="instructions_en" :value="__('messages.instructions_en')" />
                <textarea id="instructions_en" name="instructions_en" rows="5" class="w-full rounded-xl border border-slate-200 bg-white/80 px-4 py-2 text-sm text-slate-700">{{ old('instructions_en', $paymentMethod->instructions_en) }}</textarea>
            ...
            <div>
                ...
                @if ($paymentMethod->icon_path)
                    <p class="mt-2 text-xs text-slate-500">{{ __('messages.current_icon_saved') }}</p>
                @endif
            </div>
            ...
            <div class="flex items-center gap-3 text-sm text-slate-600">
                <input id="is_active" name="is_active" type="checkbox" value="1" class="rounded border-slate-300 text-emerald-600 focus:ring-emerald-500" {{ $paymentMethod->is_active ? 'checked' : '' }}>
