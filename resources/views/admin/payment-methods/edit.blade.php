...
@section('content')
    <div class="w-full rounded-3xl border border-emerald-100 bg-white p-8 shadow-sm">
        <h1 class="text-2xl font-semibold text-emerald-700">تعديل طريقة دفع</h1>
        <p class="mt-2 text-sm text-slate-600">تحديث بيانات الطريقة وتعليماتها.</p>
        ...
        <form method="POST" action="{{ route('admin.payment-methods.update', $paymentMethod) }}" enctype="multipart/form-data" class="mt-6 space-y-4">
            ...
            <div>
                ...
                @if ($paymentMethod->icon_path)
                    <p class="mt-2 text-xs text-slate-500">الأيقونة الحالية محفوظة.</p>
                @endif
            </div>
            ...
            <div class="flex gap-3">
                ...
                <a href="{{ route('admin.payment-methods.index') }}" class="rounded-full border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:border-emerald-200">عودة</a>
            </div>
        </form>
    </div>
@endsection