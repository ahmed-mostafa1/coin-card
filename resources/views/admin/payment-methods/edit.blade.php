...
@section('title', __('messages.edit_payment_method'))
...
@section('content')
    <div class="w-full rounded-3xl border border-emerald-100 dark:border-slate-700 bg-white dark:bg-slate-800 p-8 shadow-sm transition-colors duration-200">
        <h1 class="text-2xl font-semibold text-emerald-900 dark:text-emerald-100">{{ __('messages.edit_payment_method') }}</h1>
        <p class="mt-2 text-sm text-slate-900 dark:text-slate-50 dark:text-slate-100">{{ __('messages.update_payment_method_details') }}</p>
        ...
        <form method="POST" action="{{ route('admin.payment-methods.update', $paymentMethod) }}" enctype="multipart/form-data" class="mt-6 space-y-4">
            ...
            <div>
                ...
                <x-text-input id="name" name="name" type="text" :value="old('name', $paymentMethod->name)" required />
                <x-input-error :messages="$errors->get('name')" />
            </div>

            <div>
                <x-input-label for="name_en" :value="__('messages.payment_method_name_en')" />
                <x-text-input id="name_en" name="name_en" type="text" :value="old('name_en', $paymentMethod->name_en)" />
                <x-input-error :messages="$errors->get('name_en')" />
            </div>

            <div>
                <x-input-label for="slug" :value="__('messages.slug')" />
                <x-text-input id="slug" name="slug" type="text" :value="old('slug', $paymentMethod->slug)" required />
                <x-input-error :messages="$errors->get('slug')" />
            </div>

            <div>
                <x-input-label for="instructions" :value="__('messages.instructions_ar')" />
                <textarea id="instructions" name="instructions" rows="5" class="w-full rounded-xl border border-slate-50 dark:border-slate-900 bg-white dark:bg-slate-700 px-4 py-2 text-sm text-slate-700 dark:text-white shadow-sm transition focus:border-emerald-500 focus:ring-emerald-500">{{ old('instructions', $paymentMethod->instructions) }}</textarea>
                <x-input-error :messages="$errors->get('instructions')" />
            </div>

            <div>
                <x-input-label for="instructions_en" :value="__('messages.instructions_en')" />
                <textarea id="instructions_en" name="instructions_en" rows="5" class="w-full rounded-xl border border-slate-50 dark:border-slate-900 bg-white dark:bg-slate-700 px-4 py-2 text-sm text-slate-700 dark:text-white shadow-sm transition focus:border-emerald-500 focus:ring-emerald-500">{{ old('instructions_en', $paymentMethod->instructions_en) }}</textarea>
            ...
            <div>
                ...
                <x-text-input id="account_number" name="account_number" type="text" :value="old('account_number', $paymentMethod->account_number)" required />
            ...
            <div class="flex flex-col gap-3 text-sm text-slate-900 dark:text-slate-100">
                ...
                <label class="flex items-center gap-3">
                    <input id="require_transfer_proof" name="require_transfer_proof" type="checkbox" value="1"
                        class="rounded border-slate-50 dark:border-slate-900 bg-white dark:bg-slate-700 text-emerald-600 focus:ring-emerald-500"
                        {{ old('require_transfer_proof', $paymentMethod->require_transfer_proof ?? true) ? 'checked' : '' }}>
            ...
            <div class="flex items-center gap-3 text-sm text-slate-900 dark:text-slate-100">
                <input id="is_active" name="is_active" type="checkbox" value="1" class="rounded border-slate-50 dark:border-slate-900 bg-white dark:bg-slate-700 text-emerald-600 focus:ring-emerald-500" {{ $paymentMethod->is_active ? 'checked' : '' }}>
                <label for="is_active">{{ __('messages.activate_payment_method') }}</label>
            </div>

            <div>
                <x-input-label for="sort_order" :value="__('messages.sort_order')" />
                <x-text-input id="sort_order" name="sort_order" type="number" min="0" :value="old('sort_order', $paymentMethod->sort_order)" />
                <x-input-error :messages="$errors->get('sort_order')" />
            </div>

            @include('admin.payment-methods.currencies', ['paymentMethod' => $paymentMethod, 'currencies' => $currencies])
                ...
                document.addEventListener('DOMContentLoaded', () => {
                    ...
                    const buildFieldRow = (index) => `
                        <div class="rounded-2xl border border-slate-50 dark:border-slate-700 p-4" data-field-row>
                            <div class="grid gap-4 lg:grid-cols-4">
                                <div class="lg:col-span-2">
                                    <label class="text-sm font-semibold text-slate-900 dark:text-slate-50">{{ __('messages.field_label') }}</label>
                                    ...
                                    <label class="mt-2 block text-sm font-semibold text-slate-900 dark:text-slate-50">{{ __('messages.field_label') }} (EN)</label>
            ...
            <div class="flex gap-3">
                ...
                <a href="{{ route('admin.payment-methods.index') }}" class="rounded-full border border-slate-50 dark:border-slate-900 px-4 py-2 text-sm font-semibold text-slate-900 dark:text-slate-50 transition hover:border-emerald-200 dark:hover:border-emerald-500">{{ __('messages.cancel') }}</a>
            </div>
        </form>
    </div>
    ...
    <div class="mt-6 w-full rounded-3xl border border-emerald-100 dark:border-slate-700 bg-white dark:bg-slate-800 p-6 shadow-sm transition-colors duration-200">
        <div class="flex items-center justify-between">
            <h2 class="text-lg font-semibold text-emerald-900 dark:text-emerald-100">{{ __('messages.payment_method_buttons') }}</h2>
            <a href="{{ route('admin.payment-methods.buttons.create', $paymentMethod) }}" class="text-sm text-emerald-900 dark:text-emerald-100 hover:text-emerald-800 dark:hover:text-emerald-100">+ {{ __('messages.add_button') }}</a>
        </div>
                ...
                <div class="rounded-2xl border border-slate-50 dark:border-slate-700 p-3">
                    <div class="flex items-center justify-between gap-3">
                        ...
                        <a href="{{ route('admin.payment-methods.buttons.edit', [$paymentMethod, $button]) }}"
                            class="flex-shrink-0 text-xs text-emerald-900 dark:text-emerald-100 hover:text-emerald-800 dark:hover:text-emerald-100">
                            {{ __('messages.edit') }}
                        </a>
                    </div>
                </div>
            @empty
                <p class="text-sm text-slate-900 dark:text-slate-100">{{ __('messages.no_buttons_yet') }}</p>
            @endforelse
        </div>
    </div>
@endsection
