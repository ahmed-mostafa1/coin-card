    ...
    <x-card :hover="false">
        <x-page-header :title="$title">
            <a href="{{ route('admin.providers.index') }}"
               class="inline-flex items-center rounded-xl border border-slate-50 dark:border-slate-700 px-4 py-2 text-sm font-medium text-slate-900 dark:text-slate-50 hover:bg-slate-50 dark:hover:bg-slate-700 transition">
                ← العودة للمزودين
            </a>
        </x-page-header>
        ...
        <form method="POST" action="{{ $action }}" id="provider-form" class="mt-6 space-y-6">
            ...
            <div class="rounded-xl border border-slate-50 dark:border-slate-700 p-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-slate-900 dark:text-slate-50 mb-1">اسم المزود *</label>
                        <x-text-input name="name" :value="old('name', $provider?->name)" class="w-full" required />
                        <x-input-error :messages="$errors->get('name')" />
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-900 dark:text-slate-50 mb-1">المعرّف (Slug) *</label>
                        <x-text-input name="slug" :value="old('slug', $provider?->slug)" dir="ltr" class="w-full" required />
                        <x-input-error :messages="$errors->get('slug')" />
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-900 dark:text-slate-50 mb-1">الرابط الأساسي *</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-900 dark:text-slate-50 mb-1">مهلة الاتصال (ثانية)</label>
                        <x-text-input name="timeout" type="number" min="5" max="120" :value="old('timeout', $provider?->timeout ?? 25)" class="w-full" />
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-sm font-medium text-slate-900 dark:text-slate-50 mb-1">ملاحظات داخلية</label>
                        <textarea name="notes" rows="2" class="w-full rounded-xl border border-slate-50 dark:border-slate-700 bg-white dark:bg-slate-800 px-3 py-2 text-sm text-slate-700 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500">{{ old('notes', $provider?->notes) }}</textarea>
                    </div>
                    <div class="flex items-center gap-2">
                        <input type="checkbox" name="is_active" id="is_active" value="1"
                               {{ old('is_active', $provider?->is_active) ? 'checked' : '' }}
                               class="rounded border-slate-50 text-emerald-600">
                        <label for="is_active" class="text-sm font-medium text-slate-900 dark:text-slate-50">مفعّل</label>
                    </div>
                </div>
            </div>
            ...
            <div class="rounded-xl border border-slate-50 dark:border-slate-700 p-5">
                ...
                <div class="mb-4">
                    <label class="block text-sm font-medium text-slate-900 dark:text-slate-50 mb-1">نوع المصادقة *</label>
                ...
                <div id="auth-apikey" class="grid grid-cols-1 md:grid-cols-2 gap-4 auth-fields">
                    <div>
                        <label class="block text-sm font-medium text-slate-900 dark:text-slate-50 mb-1">API Key</label>
                        <x-text-input name="cred_key" dir="ltr" class="w-full" placeholder="{{ $isEdit ? '(فارغ = الإبقاء على القيمة الحالية)' : '' }}" />
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-900 dark:text-slate-50 mb-1">API Secret</label>
                ...
                <div id="auth-basic" class="grid grid-cols-1 md:grid-cols-2 gap-4 auth-fields hidden">
                    <div>
                        <label class="block text-sm font-medium text-slate-900 dark:text-slate-50 mb-1">اسم المستخدم</label>
                        <x-text-input name="cred_username" dir="ltr" class="w-full" />
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-900 dark:text-slate-50 mb-1">كلمة المرور</label>
                        <x-text-input name="cred_password" type="password" dir="ltr" class="w-full" />
                    </div>
                </div>
                <div id="auth-bearer" class="auth-fields hidden">
                    <label class="block text-sm font-medium text-slate-900 dark:text-slate-50 mb-1">Bearer Token</label>
            ...
            <div class="rounded-xl border border-slate-50 dark:border-slate-700 p-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-slate-900 dark:text-slate-50 mb-1">مسار قائمة المنتجات *</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-900 dark:text-slate-50 mb-1">طريقة الطلب</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-900 dark:text-slate-50 mb-1">نوع التصفح</label>
                    ...
                    <div id="pagination-params" class="{{ old('catalog_pagination_type', $provider?->catalog_pagination_type ?? 'none') === 'none' ? 'hidden' : '' }} md:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-slate-900 dark:text-slate-50 mb-1">اسم معامل الصفحة</label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-slate-900 dark:text-slate-50 mb-1">اسم معامل الحجم</label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-slate-900 dark:text-slate-50 mb-1">حجم الصفحة</label>
            ...
            <div class="rounded-xl border border-slate-50 dark:border-slate-700 p-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-900 dark:text-slate-50 mb-1">السعر (price) *</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-900 dark:text-slate-50 mb-1">المعرّف الخارجي</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-900 dark:text-slate-50 mb-1">الوصف</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-900 dark:text-slate-50 mb-1">النوع</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-900 dark:text-slate-50 mb-1">التوفر</label>
            ...
            <div class="rounded-xl border border-slate-50 dark:border-slate-700 p-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-900 dark:text-slate-50 mb-1">مسار معرّف المعاملة في الاستجابة</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-900 dark:text-slate-50 mb-1">مسار حالة التنفيذ في الاستجابة</label>
            ...
            <div class="rounded-xl border border-slate-50 dark:border-slate-700 p-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-900 dark:text-slate-50 mb-1">اسم معامل رقم الطلب</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-900 dark:text-slate-50 mb-1">قيم النجاح (مفصولة بفواصل)</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-900 dark:text-slate-50 mb-1">قيم الفشل (مفصولة بفواصل)</label>
            ...
            @if($isEdit)
                <div class="rounded-xl border border-emerald-200 dark:border-emerald-800 p-5">
                    ...
                    <div id="test-result" class="mt-4 hidden">
                        ...
                        <pre id="test-preview" class="bg-slate-50 dark:bg-slate-800 text-xs p-3 rounded-xl overflow-x-auto hidden"></pre>
                    </div>
                </div>
            @endif
