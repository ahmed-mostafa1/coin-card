...
@section('content')
    <div class="w-full rounded-3xl border border-emerald-100 bg-white p-8 shadow-sm dark:border-slate-700 dark:bg-slate-800 transition-colors duration-200">
        ...
        @if (session('status'))
            <div class="mt-4 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-900 dark:bg-emerald-800 dark:border-emerald-700 dark:text-emerald-300">
                {{ session('status') }}
            </div>
        @endif
