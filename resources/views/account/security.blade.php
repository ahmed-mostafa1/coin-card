            ...
            <div class="mt-6 rounded-2xl border border-slate-200 p-5 dark:border-slate-700">
                <div class="flex flex-wrap items-center justify-between gap-3">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-slate-500">سيتم إرسال رمز إلى بريدك الإلكتروني بعد كلمة المرور عند تسجيل الدخول.</p>
                    </div>
                    <span class="rounded-full px-3 py-1 text-xs font-semibold {{ auth()->user()->two_factor_email_enabled ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600' }}">
                ...
                @else
                    <form method="POST" action="{{ route('account.security.2fa.disable') }}" class="mt-4 max-w-md space-y-3">
                        ...
                        <x-input-error :messages="$errors->get('current_password')" />
                        <button type="submit" class="rounded-xl border border-rose-200 px-4 py-2 text-sm font-semibold text-rose-700 hover:bg-rose-50">تعطيل التحقق الثنائي</button>
                    </form>
                @endif
            </div>
        </x-card>
                ...
                @forelse ($recentLogs as $log)
                    <div class="rounded-xl border border-slate-200 p-3 text-sm dark:border-slate-700">
                        <div class="flex flex-wrap items-center justify-between gap-2">
                            <span class="font-semibold text-slate-700 dark:text-slate-200">{{ $log->action }}</span>
                            <span class="text-xs text-slate-400">{{ $log->created_at->format('Y-m-d H:i') }}</span>
                        </div>
                        <p class="mt-1 text-xs text-slate-500">IP: {{ $log->ip_address ?? '-' }} | {{ $log->device_type ?? '-' }} | {{ $log->browser ?? '-' }}</p>
                    </div>
                @empty
                    <p class="text-sm text-slate-500">لا توجد سجلات بعد.</p>
                @endforelse
            </div>
        </x-card>
    </div>
