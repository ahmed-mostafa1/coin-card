    ...
    <div class="space-y-6">
            ...
            <div class="mt-6 rounded-2xl border border-slate-50 p-5 dark:border-slate-700">
                <div class="flex flex-wrap items-center justify-between gap-3">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-slate-900 dark:text-slate-400">سيتم إرسال رمز إلى بريدك الإلكتروني بعد كلمة المرور عند تسجيل الدخول.</p>
                    </div>
                    <span class="rounded-full px-3 py-1 text-xs font-semibold {{ auth()->user()->two_factor_email_enabled ? 'bg-emerald-100 text-emerald-900 dark:bg-emerald-900/30 dark:text-emerald-300' : 'bg-slate-50 text-slate-900 dark:bg-slate-700 dark:text-slate-50' }}">
        ...
        <x-card :hover="false" class="p-8">
            ...
            <div class="mt-4 space-y-3">
                ...
                @empty
                    <p class="text-sm text-slate-900 dark:text-slate-400">لا توجد سجلات بعد.</p>
                @endforelse
            </div>
        </x-card>
    </div>
