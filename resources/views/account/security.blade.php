...
@section('content')
    <div class="space-y-6">
        <x-card :hover="false" class="p-8">
            ...
            @if (session('status'))
                <div class="mt-4 rounded-xl border border-emerald-200 dark:border-emerald-800 bg-emerald-50 dark:bg-emerald-900/30 px-4 py-3 text-sm text-emerald-900 dark:text-emerald-100">{{ session('status') }}</div>
            @endif

            <div class="mt-6 rounded-2xl border border-slate-50 p-5 dark:border-slate-700">
                <div class="flex flex-wrap items-center justify-between gap-3">
                    <div>
                        <h2 class="text-lg font-semibold text-slate-800 dark:text-white">التحقق الثنائي عبر البريد</h2>
                        <p class="mt-1 text-sm text-slate-900 dark:text-slate-100">سيتم إرسال رمز إلى بريدك الإلكتروني بعد كلمة المرور عند تسجيل الدخول.</p>
                    </div>
                    <span class="rounded-full px-3 py-1 text-xs font-semibold {{ auth()->user()->two_factor_email_enabled ? 'bg-emerald-100 text-emerald-900 dark:bg-emerald-900/30 dark:text-emerald-100' : 'bg-slate-50 text-slate-900 dark:bg-slate-700 dark:text-slate-50' }}">
                ...
                @forelse ($recentLogs as $log)
                    <div class="rounded-xl border border-slate-50 p-3 text-sm dark:border-slate-700">
                        <div class="flex flex-wrap items-center justify-between gap-2">
                            ...
                            <span class="text-xs text-slate-100">{{ $log->created_at->format('Y-m-d H:i') }}</span>
                        </div>
                        <p class="mt-1 text-xs text-slate-900 dark:text-slate-50">IP: {{ $log->ip_address ?? '-' }} | {{ $log->device_type ?? '-' }} | {{ $log->browser ?? '-' }}</p>
                    </div>
                @empty
                    <p class="text-sm text-slate-900 dark:text-slate-100">لا توجد سجلات بعد.</p>
                @endforelse
            </div>
        </x-card>
    </div>
