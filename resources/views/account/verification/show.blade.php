    ...
    <x-card :hover="false" class="p-8">
        ...
        <div class="mt-6 rounded-2xl border border-slate-200 p-4 dark:border-slate-700">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="font-semibold text-slate-800 dark:text-white">{{ auth()->user()->is_verified ? 'حسابك موثق' : 'حسابك غير موثق' }}</p>
                    @if ($verificationRequest)
                        <p class="text-sm text-slate-500">آخر طلب: {{ $verificationRequest->status }}</p>
                        @if ($verificationRequest->review_note)
                            <p class="mt-1 text-sm text-amber-700">ملاحظة الإدارة: {{ $verificationRequest->review_note }}</p>
                        @endif
                    @endif
                </div>
            </div>
        ...
        @if ($fields->isEmpty())
            <div class="mt-6 rounded-2xl border border-dashed border-slate-200 p-8 text-center text-sm text-slate-500">نموذج التوثيق غير متاح حالياً.</div>
        @else
            <form method="POST" action="{{ route('account.verification.store') }}" enctype="multipart/form-data" class="mt-6 space-y-4">
                @csrf
                @foreach ($fields as $field)
                    <div class="space-y-2">
                        <x-input-label :for="$field->name_key" :value="$field->localized_label" />
                        @if ($field->type === 'textarea')
                            <textarea id="{{ $field->name_key }}" name="fields[{{ $field->name_key }}]" rows="3" class="store-input" placeholder="{{ $field->localized_placeholder }}" @required($field->is_required)>{{ old('fields.'.$field->name_key) }}</textarea>
                        @elseif ($field->type === 'select')
                            <x-select id="{{ $field->name_key }}" name="fields[{{ $field->name_key }}]" :required="$field->is_required">
                                <option value="">اختر</option>
                                @foreach (($field->options ?? []) as $option)
                                    <option value="{{ $option }}" @selected(old('fields.'.$field->name_key) === $option)>{{ $option }}</option>
                                @endforeach
                            </x-select>
                        @elseif ($field->type === 'file' || $field->type === 'image')
                            <input id="{{ $field->name_key }}" name="fields[{{ $field->name_key }}]" type="file" class="store-input" accept="{{ $field->type === 'image' ? 'image/*' : '.jpg,.jpeg,.png,.webp,.pdf,image/*,application/pdf' }}" @required($field->is_required)>
                        @elseif ($field->type === 'camera')
                            <div class="rounded-2xl border border-slate-200 p-3 dark:border-slate-700" data-camera-field>
                                <input id="{{ $field->name_key }}" name="fields[{{ $field->name_key }}]" type="hidden" data-camera-input @required($field->is_required)>
